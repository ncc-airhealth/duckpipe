"""
[description]
Military Demarcation Line (MDL) calculator that computes per-feature minimum distances to MDL
...
VALID_YEARS = [2000, 2005, 2010, 2015, 2020, 2023]
TABLE_NAME = "mdl"
VAR_NAME_MACRO = """
...
def _generate_query(year: int, table_path: Path) -> Tuple[str, str, str]:
    ...
    main_query = f"""
        ...
        result AS (
            ...
            FROM chunk AS c, mdl_sel_year AS a
            GROUP BY c.id
        )
        SELECT * FROM result;
    """
...
class MDLDistanceCalculator:

    @typechecked
    def calculate_mdl_distance(self, years: int | list[int]) -> Self:
        """
        [description]
        Calculate per-feature minimum distance to the MDL for one or more years.
        ...
        [example usage]
        ```python
        calculator.calculate_mdl_distance(years=[2020, 2021, 2023])
        ```
        """
        # normalize input
